...
pnpm gen:music    # Scan public/audio/<Artist - Album>/ folders -> src/data/generatedAlbums.json
...
- **Catalog** (`src/data/music.ts`) = **real albums** (scanned from `public/audio/<Artist - Album>/` by `pnpm gen:music` into `generatedAlbums.json`) merged **ahead of** the remaining gradient **dummies** (`mockMusic.ts`). So real albums lead the library grid; as each dummy slot is replaced, drop it from `mockMusic`. To add a real album: drop a folder named `Artist - Album (YYYY)` into `public/audio/` (the trailing `(YYYY)` is **optional** — it sets the release year and wins over the ID3 tag, which is often a reissue date; without it the year falls back to ID3 → current year) containing a cover image (`cover.<webp|jpg|png>`) + numbered tracks (`1.Title.mp3`, `2.Title.mp3`, …), run `pnpm gen:music`, commit the audio + the regenerated JSON. The album id is derived from `Artist - Album` only (year-independent, so adding a year later keeps ids stable). Durations are estimated from the MP3 header (the engine confirms the real value via `loadedmetadata`).
- **Per-album accent** is **extracted from the cover image at runtime** (`useCoverAccent` → `extractCoverAccent`), cached in the store (`coverAccents[albumId]`), and applied as `--accent` (dark-neon only) for UI + the canvas visualizer. Gradient dummies skip extraction and keep their static `accent`. A monochrome cover (e.g. a B&W photo) honestly resolves to a neutral grey.
- **Asset URLs**: `cover`/`src` are stored as RAW relative paths and resolved once via `resolveAssetUrl` (`src/lib/resolveAssetUrl.ts`) = `BASE_URL + encodeURI(path)`, so paths with spaces, commas, apostrophes, or unicode (e.g. `…`) load on both Vercel and the `/Vinyl-Player/` Pages base.

## Directory map

```
scripts/generate-catalog.mjs  # folder scanner -> src/data/generatedAlbums.json (pnpm gen:music)
src/
  App.tsx                     # mounts hidden <audio>, usePlayer, view switch
  main.tsx
  store/usePlayerStore.ts     # the single store
  hooks/                      # usePlayer, useAudioVisualizer, useCoverAccent, usePrefersReducedMotion, useMediaKeyboard
  lib/audio/                  # AudioEngine (Web Audio graph), formatTime
  lib/color/extractAccent.ts  # runtime dominant-color extraction from a cover image
  lib/resolveAssetUrl.ts      # RAW relative path -> BASE_URL + encodeURI (audio + cover)
  ...
  data/music.ts               # active catalog: generatedAlbums.json (real) + mockMusic.ts (dummies)
  data/generatedAlbums.json   # GENERATED by gen:music — do not hand-edit
  styles/                     # globals.css (@theme bridge + base), animations.css (keyframes + reduced-motion)
public/audio/<Artist - Album (YYYY)>/  # real album folders (cover image + numbered N.Title.mp3); (YYYY) optional
public/{locales}              # runtime i18n JSON
```

## Conventions

- **Path alias `@/`** → `src/` (e.g. `import { formatTime } from '@/lib/audio/formatTime'`).
- **Consume theme tokens via Tailwind utilities** (`bg-bg`, `text-text`, `text-accent`, `bg-surface`, `text-muted`, `border-border`, `font-display`, `rounded-card`, `shadow-card`). **Never hardcode hex colors in components.** The per-album accent (extracted from the cover at runtime, see Architecture) is injected as `style={{ '--accent': accent }}` on the player root; the canvas visualizer reads the same value from `coverAccents`.
